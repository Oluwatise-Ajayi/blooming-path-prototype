...
export default function EvidenceTrailModal({ isOpen, onClose, candidateName, capabilityName, score, transcriptSnippet }) {
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl max-w-2xl w-full p-6 shadow-2xl overflow-hidden relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-on-surface-variant">
                Derivation audit for <strong className="text-on-surface">{candidateName || 'Amina Hassan'}</strong> — {capabilityName || 'Problem solving'}
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="my-4 p-3 rounded-xl bg-surface-container-low border border-primary-fixed text-center">
          <p className="text-xs font-semibold text-primary flex items-center justify-center gap-2 flex-wrap">
            <span>Source Interaction</span>
            <span className="text-outline">➔</span>
            <span>Transcript</span>
            <span className="text-outline">➔</span>
            <span>Observed Behaviour</span>
            <span className="text-outline">➔</span>
            <span>Capability Rubric</span>
            <span className="text-outline">➔</span>
            <span className="bg-secondary-container px-2 py-0.5 rounded text-on-secondary-container font-bold">Readiness Profile</span>
          </p>
        </div>

        {/* Step-by-Step Chain Grid */}
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
          {/* Step 1: Individual Interaction */}
          <div className="p-4 rounded-xl bg-surface border border-outline-variant relative">
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1.5">
                ...
                Individual Workplace Interaction
              </span>
              <span className="text-[11px] text-on-surface-variant bg-surface-container-high px-2 py-0.5 rounded">Administrative Assistant Simulation</span>
            </div>
            <p className="text-xs text-on-surface-variant italic">
              "Assessed during Appointment Scheduling workplace simulation task."
            </p>
          </div>

          {/* Step 2: Captured Transcript */}
          <div className="p-4 rounded-xl bg-surface border border-outline-variant relative">
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-secondary font-semibold">Verified Interaction Source</span>
            </div>
            <div className="p-3 rounded-lg bg-surface-container-lowest border border-outline-variant text-xs text-on-surface font-mono">
              "{transcriptSnippet || 'I understand you need 10am, but that time is already booked. I can offer 11:30 instead or check another day for you.'}"
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-surface border border-outline-variant relative">
            ...
            <ul className="space-y-1.5 text-xs">
              <li className="flex items-center gap-2 text-on-surface">
                ...
                Recognised 10:00 schedule conflict
              </li>
              <li className="flex items-center gap-2 text-on-surface">
                <span className="material-symbols-outlined text-secondary text-[16px]">check_circle</span>
                Explained operational schedule constraint politely
              </li>
              <li className="flex items-center gap-2 text-on-surface">
                <span className="material-symbols-outlined text-secondary text-[16px]">check_circle</span>
                Proposed practical alternative time slot (11:30)
              </li>
            </ul>
          </div>

          {/* Step 4: Capability & Assessment State */}
          <div className="p-4 rounded-xl bg-gradient-to-r from-surface-container-low to-secondary-container/20 border border-secondary-container flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-on-surface">Capability Assessment State</p>
              <p className="text-xs text-on-surface-variant">Validated against defined workplace capability rubrics.</p>
            </div>
            <div className="text-right">
              <span className="text-base font-extrabold text-secondary capitalize">{score || 'Demonstrated'}</span>
              <p className="text-[10px] font-bold text-on-secondary-container uppercase">Confidence: Medium</p>
            </div>
          </div>
        </div>
        ...
        <div className="pt-4 mt-4 border-t border-outline-variant flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-lg bg-primary text-on-primary font-semibold text-xs hover:opacity-90 transition-colors"
          >
